...
import { Calendar, Users, CheckCircle2, Circle, FileText, Clock, TrendingUp, Target, Sparkles, Loader2 } from 'lucide-react'
...
export function QBRTab({ account }: QBRTabProps) {
  ...
  const handleGenerateWithClaude = () => {
    ...
    setTimeout(() => {
      const content = `## QBR Talking Points for ${account.name}

### Opening (5 min)
- Thank team for continued partnership
- Acknowledge ${account.lifecycleStage === 'Renewal' ? 'long-standing relationship' : 'growth journey'} together
- Preview key topics: Usage trends, success stories, expansion opportunities

### Business Value Review (10 min)
- **Current ARR**: $${(account.arr / 1000).toFixed(0)}K
- **Health Score**: ${healthScore.overall}/100 (${healthScore.trend})
${useCases.length > 0 ? `- **Documented ROI**: ${useCases[0].roi || 'Pending calculation'}%` : ''}
${apiGrowth !== 0 ? `- **API Usage Growth**: ${apiGrowth >= 0 ? '+' : ''}${apiGrowth}% MoM` : ''}

### Product Adoption Deep-Dive (15 min)
- Current seat utilization: ${seatUtilization}%
${seatUtilization < 70 ? `- **Action**: Discuss strategies to increase adoption` : '- Strong adoption across teams'}
- Feature adoption highlights to discuss
- Upcoming features relevant to their use cases

### Expansion Discussion (10 min)
${totalPipelineValue > 0 ?
`- Active pipeline: $${(totalPipelineValue / 1000).toFixed(0)}K
- Key opportunities to advance this quarter` :
'- Identify new expansion opportunities based on usage patterns'}

### Next Steps & Action Items (5 min)
- Confirm follow-up meeting cadence
- Assign owners to action items
- Set goals for next quarter

---
*Generated by Claude • Review and customize before use*`
                ...
                {generatedContent && (
                  <div className="mt-4 p-4 bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200 rounded-lg">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-sm font-semibold text-orange-800">Claude-Generated Content</span>
                    </div>
                    <div className="prose prose-sm max-w-none">
                      <pre className="whitespace-pre-wrap text-xs bg-white/60 p-3 rounded-md font-sans">
                        {generatedContent}
                      </pre>
                    </div>
